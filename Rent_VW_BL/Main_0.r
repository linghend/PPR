#####################################################################
# Function: Main
# Description: Main Entry of Vacant-Weighted Building-Level Rent
#
# Developer: Sam He
...
for (PTID in (1:4))
{
  for (CBSAID in (CostarCBSA$CBSAID))
  {
    ...
    TData<-RentVWDataRaw[RentVWDataRaw$cbsacode==CBSAID & RentVWDataRaw$proptype==Ptypes[PTID],]
    
    ROutput<-cbind(as.character(TData$quarter),TData$rent)
    RCBSA<-c('CBSAID',CBSAID)    
    RCoef<-rbind(RCBSA,ROutput)      
    colnames(RCoef)<-c("Quarters","Rent_VW")
      write.table(RCoef,paste("./Results/",Ptypes[PTID],'/',as.character(CostarCBSA[CostarCBSA$CBSAID==CBSAID,"GeographyName"]),"_",Ptypes[PTID],"_RentVWBL_",CBSAID,"_2011Q4.csv",sep=""),sep=",",row.names=FALSE)
  }
}
