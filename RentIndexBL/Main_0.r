...
rm(list=ls())
#setwd("C:\\SamHe\\git\\PPR\\RentIndexBL")
...
formula0=log(AskingRate)~BldgClass+SubmarketName+QuarterOff
...
for (PTID in (1:4))
{
  for (CBSAID in (CostarCBSA$CBSAID))
  {
    ...
    RData<-DataClean(TData)

    
    if (nrow(RData)>=1000)
    {   
      formula=formula0
      if(nlevels(RData$SubmarketName)==0) formula=log(AskingRate)~BldgClass+QuarterOff
      ...
      RDatalw_W=nb2listw(RDatanb, style="W",zero.policy=TRUE,glist=RDatanbd_inv)
      ...
      write.table(RCoef,paste("./Results/",Ptypes[PTID],'/',as.character(CostarCBSA[CostarCBSA$CBSAID==CBSAID,"GeographyName"]),"_",Ptypes[PTID],"_ListingRentIndex_",CBSAID,"_2011Q4.csv",sep=""),sep=",")
      
      
    }else{
      print(sprintf("There are only %i quarterly minimum usable records avaiable. Ignore analysis!",nrow(RData)))
    }
  }
}
